...
import { Send, Bot, ChevronDown, Square, Brain, Minimize2, BarChart3, Shield, Lightbulb, Eye, Rocket, Zap } from 'lucide-react';
import { api } from './api';
...
export function ChatView({ initialConversationId, onConversationCreated, project }: ChatViewProps) {
  ...
  const [modes, setModes] = useState<{ id: string; name: string; description: string; icon: string }[]>([]);
      ...
      >
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-tertiary mt-1.5 mb-5">Assign tasks, spawn agents, or pick a mode</p>
            {modes.length > 0 && (
              <div className="grid grid-cols-2 gap-2.5 max-w-sm w-full">
                {modes.map(mode => {
                  const IconMap: Record<string, typeof Shield> = { shield: Shield, lightbulb: Lightbulb, eye: Eye, rocket: Rocket };
                  const Icon = IconMap[mode.icon] || Zap;
                  return (
                    <button
                      key={mode.id}
                      onClick={() => {
                        const prefix = `/${mode.id} ${project || ''}`.trim();
                        setInput(prefix);
                        textareaRef.current?.focus();
                      }}
                      className="flex items-start gap-3 p-3 rounded-xl border border-border bg-card hover:bg-subtle hover:border-accent/30 transition-all text-left group"
                    >
                      <div className="p-1.5 rounded-lg bg-accent/10 text-accent group-hover:bg-accent/20 transition-colors flex-shrink-0 mt-0.5">
                        <Icon size={16} />
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-primary">/{mode.id}</p>
                        <p className="text-[11px] text-tertiary mt-0.5 leading-snug">{mode.description}</p>
                      </div>
                    </button>
                  );
                })}
